...
{
    public class PassiveAbility_PinocchioBoyM : PassiveAbilityBase
    {
        private AudioClip[] _oldEnemytheme;
        private SephirahType sephirah;
        public override void OnWaveStart()
        {
            Singleton<StageController>.Instance.AddEgoMapByAssimilation("Pinocchio");
            SingletonBehavior<BattleCamManager>.Instance.SetCreatureFilter();
            MapManager component = Util.LoadPrefab("CreatureMaps/CreatureMap_Pinocchio", SingletonBehavior<BattleSceneRoot>.Instance.transform).GetComponent<MapManager>();
